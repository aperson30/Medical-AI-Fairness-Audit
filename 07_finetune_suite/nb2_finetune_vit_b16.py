# ============================================================
# NOTEBOOK 2 — Fine-Tune ViT-B/16 (single T4)
# Dataset: nazmusresan/fitzpatrick17k
# GPU T4 x1, Internet ON
# Expected runtime: ~2-2.5 hours total (smaller model than CLIP-L/14)
#
# Mirrors nb_finetune_v3_single_gpu.py exactly, but with ViT-B/16 backbone
# instead of CLIP. Same bug fix (real 200-image dark pool), same seeds,
# same DRO/SMOTE protocol, same effective batch size.
# ============================================================
...
CFG = dict(
    ...
    RESULTS_DIR       = 'results_ft_vit',
    RAW_PREDS_DIR     = 'results_ft_vit/raw_preds',
    ...
    FT_LAST_N_BLOCKS  = 4,
    BATCH_SIZE        = 16,           # ViT-B is smaller, can use larger batch
    GRAD_ACCUM        = 2,
    EVAL_BATCH_SIZE   = 32,
    EPOCHS            = 5,
    LR_HEAD           = 1e-4,
...
print(f'Total: {len(df)} | Light: {len(light_df)} | Dark: {len(dark_df)} '
...
class ViTFineTuned(nn.Module):
    def __init__(self, vit_model, num_classes=3, dropout=0.3, ft_last_n=4):
        super().__init__()
        self.vit = vit_model
        hidden_size = vit_model.config.hidden_size  # 768

        for p in self.vit.parameters():
            p.requires_grad = False
        n_layers = len(self.vit.encoder.layer)
...
# ── Helpers (identical to nb_finetune_v3) ─────────────────────
...
processor = ViTImageProcessor.from_pretrained(CFG['MODEL_ID'])
...
for seed in CFG['SEEDS']:
    ...
    intv_test_loader = DataLoader(test_subset, batch_size=eval_dl,
                                  shuffle=False, num_workers=2, pin_memory=True)

    for intervention in INTERVENTIONS:
        print(f'\n── {intervention.upper()} (seed {seed}) ──')
        t0 = time.time()
        vit_base = ViTModel.from_pretrained(CFG['MODEL_ID'])
        model = ViTFineTuned(vit_base, num_classes=3,
                             dropout=CFG['DROPOUT'],
        ...
        try:
            ...
            pd.DataFrame(all_results).to_csv(
                os.path.join(CFG['RESULTS_DIR'], 'ft_vit_results.csv'), index=False)
...
# ── Save group weight trajectories (for figure 1) ────────────
import json
...
print(f"\n{'='*60}\nSUMMARY (ViT-B/16 fine-tune)\n{'='*60}")
df_results = pd.DataFrame(all_results)
df_results.to_csv(os.path.join(CFG['RESULTS_DIR'], 'ft_vit_results.csv'), index=False)
